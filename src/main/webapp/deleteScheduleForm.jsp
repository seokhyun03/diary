...
<%
	...
	// 디버깅(오청값 확인)
	System.out.println(scheduleNo + " <-- deleteScheduleForm parameter scheduleNo");
	// 드라이버 로딩
	Class.forName("org.mariadb.jdbc.Driver");
	// DB 접속
	Connection conn = DriverManager.getConnection("jdbc:mariadb://127.0.0.1:3306/diary", "root", "java1234");
	// schedule 테이블에 schedule_no 일치하는 데이터를 조회하는 sql 전송
	String sql = "select schedule_no scheduleNo, schedule_date scheduleDate from schedule where schedule_no = ?";
	PreparedStatement stmt = conn.prepareStatement(sql);
	stmt.setInt(1, scheduleNo); 	// 1번째 ? = scheduleNo
	// sql 디버깅
	System.out.println(stmt + " <-- deleteScheduleForm sql");
				...
				%>
					<tr>
						...
						<td>
							...
							<input type="hidden" name="scheduleDate" value="<%=s.scheduleDate %>">
